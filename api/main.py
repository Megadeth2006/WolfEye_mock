...
def get_mock_score_and_flags(resume_id: str) -> tuple[int, list[AnalysisFlag]]:
    """Возвращает мок-данные для конкретного резюме с новыми маркерами"""
    
    # Предопределенные мок-данные для каждого резюме с новыми маркерами
    mock_data = {
        "13db2fbf000df537aa00bb41f05063456f6a39": {  # Олег Устинов
            ...
            "flags": [
                AnalysisFlag(
                    ...
                    fact="ФИО корректное, образование завершено, возраст соответствует опыту, контакты валидные."
                )
            ]
        },
        "108b9793000f5a420900bb41f052543668456f": {  # Илья Воробьев
            ...
            "flags": [
                AnalysisFlag(
                    name="Критическое несоответствие возраста и опыта", 
                    fact="Возраст 23 года, но опыт 61 месяц (5+ лет). Невозможно начать работать в 18 лет с таким опытом."
                ),
                AnalysisFlag(
                    name="Проблемы с образованием", 
                    fact="Неоконченное высшее образование при большом опыте работы. Подозрительно для такого возраста."
                ),
                AnalysisFlag(
                    name="Подозрительные компании", 
                    fact="Работа в МКБ с 22 лет на позиции Team Lead выглядит нереалистично."
                ),
                AnalysisFlag(
                    name="Шаблонные описания", 
                    fact="Описания работы содержат типичные фразы и цифры, характерные для накрученных резюме."
                )
            ]
        },
        "18c8bdbe000f4ab1d300bb41f0634b4a386c33": {  # Антон Назаров
            ...
            "flags": [
                AnalysisFlag(
                    ...
                    fact="Отсутствуют ссылки на компании OmniMarket и Borealis Finance в опыте работы."
                ),
                AnalysisFlag(
                    name="Подозрительные достижения", 
                    fact="Слишком конкретные цифры улучшений (8%, 25%, 7%, 12%) выглядят как шаблонные."
                )
            ]
        },
        "201217d6000f50c6c300bb41f0474747393845": {  # Кирилл Дагданча
            ...
            "flags": [
                AnalysisFlag(
                    name="Подозрительное ФИО", 
                    fact="Фамилия 'Дагданча' выглядит необычно и может быть выдуманной."
                ),
                AnalysisFlag(
                    name="Несоответствие возраста и опыта", 
                    fact="Опыт 63 месяца при не указанном возрасте вызывает подозрения."
                ),
                AnalysisFlag(
                    name="Подозрительные компании", 
                    fact="Работа в OZON и Яндекс без указания конкретных ссылок на работодателей."
                )
            ]
        },
        "22f2b4ee000f56dc5b00bb41f0564e71743246": {  # Александр Панарин
            ...
            "flags": [
                AnalysisFlag(
                    name="Критическое несоответствие возраста и опыта", 
                    fact="Возраст 21 год, но опыт 43 месяца (3.5+ года). Невозможно начать работать в 17-18 лет."
                ),
                AnalysisFlag(
                    name="Подозрительные компании", 
                    fact="Работа в YADRO и IT_One с 18 лет на позиции разработчика выглядит нереалистично."
                ),
                AnalysisFlag(
                    name="Шаблонные описания", 
                    fact="Описания работы содержат типичные фразы и конкретные цифры улучшений."
                ),
                AnalysisFlag(
                    name="Проблемы с образованием", 
                    fact="Образование в МГТУ им. Баумана с окончанием в 2025 году, но уже есть опыт работы."
                )
            ]
        },
        "37cd74c9000f1a765200bb41f0323133427552": {  # Дмитрий Молочников
            ...
            "flags": [
                AnalysisFlag(
                    name="Проблемы с компаниями", 
                    fact="Отсутствуют ссылки на компании Eastwind и CloudFactory в опыте работы."
                ),
                AnalysisFlag(
                    name="Подозрительные достижения", 
                    fact="Слишком конкретные цифры улучшений (3 раза, 15 раз, 50 мс до 6 мс) выглядят как шаблонные."
                )
            ]
        },
        "3c07ff2f000f4e880700bb41f0435345554a79": {  # Новиков Артем
            ...
            "flags": [
                AnalysisFlag(
                    name="Проблемы с ФИО", 
                    fact="Отчество отсутствует, что снижает достоверность данных."
                ),
                AnalysisFlag(
                    name="Неоконченное образование", 
                    fact="Образование не завершено (окончание в 2025 году), но уже есть опыт работы."
                ),
                AnalysisFlag(
                    name="Несоответствие возраста и опыта", 
                    fact="Опыт 51 месяц при не указанном возрасте вызывает подозрения."
                ),
                AnalysisFlag(
                    name="Подозрительные компании", 
                    fact="Работа в Дивергент и BASF без указания конкретных ссылок на работодателей."
                )
            ]
        }
    }
